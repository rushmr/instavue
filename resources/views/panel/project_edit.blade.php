...
<div class="panel panel-info">
  ...
  <div class="panel-heading">
    <h4>Редактировать проект {{$project->name}}</h4>
  </div>
  ...
  <div class="panel-body">
	 ...
	 <form action="{{ route('project.update', ['id'=>$project->id]) }}" method="POST">
	  ...
	  <div class="form-group">
	    ...
	    <input type="text" name="login" class="form-control" value="{{ $project->login }}">
	  </div>
	  <div class="form-group">
	    <label for="password">Инстаграмм пароль</label>
	    <input type="password" name="password" class="form-control" value="{{ $project->password }}">
	  </div>
	  <div class="form-group">
	    <label for="feed">Источники контента (паблики VK)</label>
	    <textarea name="feed" id="feed" class="form-control" rows="10">{{ $feed }}</textarea>
	  </div>
	  <div class="form-group">
	    <label for="tags">Список тегов для поста</label>
	    <textarea name="tags" id="tags" class="form-control" rows="10">{{ $tags }}</textarea>
	  </div>
